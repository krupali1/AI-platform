...
- Godown's Invoice-Number-prefix exception now covers two patterns per the updated PRD (`IN-` → `IN`, `VZPL-` → `VZPL`) — surfaced as an escalation rule (blank Godown → flagged for review), never an automatic override, consistent with Godown never being rule-settable.
