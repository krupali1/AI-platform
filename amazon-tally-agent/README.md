...
- **Order_Date source is unconfirmed.** The updated PRD marks "Order Date" as struck through in favor of "Invoice Date (awaiting confirmation)" — not yet changed here since which column to actually map is still open. Also flagged: Amazon's raw MTR date format is `MM/DD/YYYY HH:MM:SS AM/PM` (a full timestamp) even though Excel often displays date-only — this pipeline doesn't currently strip the time portion, since there's no real sample of that exact raw string to confirm the fix against yet.
- **Billing_Pincode/City/State are marked "awaiting confirmation, found error"** in the updated PRD — the Bill-to/Ship-to B2B/B2C branching logic itself is unchanged, but PIL flagged a data issue with it that isn't resolved yet.
- **Blank Godown is auto-filled from the Invoice Number's prefix** (`IN-` → `IN`, `VZPL-` → `VZPL`) — confirmed and explicitly authorized by PIL, reversing an earlier "never override Godown" instruction specifically for this one PRD-documented pattern. Implemented as dedicated pipeline logic (`_infer_godown_from_invoice` in `backend/tally_pipeline.py`), not a configurable Rule — `godown` still can never be the target of an arbitrary Rule (enforced via `FORBIDDEN_ACTION_FIELDS`), so this stays the one specific, code-reviewed exception rather than opening the field back up generally. A row only escalates for review now if the Invoice Number is blank or starts with neither known prefix.
- The updated PRD's CGST/SGST remarks now read "Applicable if stock moved across different states" (previously "same state" for all three, including IGST) - this looks reversed from standard GST treatment (CGST+SGST are intra-state, IGST is inter-state) and is likely a wording slip in the client's own document. Doesn't affect this pipeline either way, since CGST/SGST/IGST are computed directly from Amazon's own per-row rate columns, never from a same-state/different-state comparison this system makes itself.
...
4. Optional: in `/admin` → **Rules**, add a `row_filter` rule (`transaction_type equals Shipment`, action `include_row`) to see the "Cancelled" row get excluded. Voucher type / Sales Ledger rules aren't needed for the Excel output at all — only add them if you also want the Tally XML download to carry a real ledger name.
